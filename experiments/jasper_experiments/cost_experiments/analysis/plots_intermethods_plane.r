...
base_directory <- c('/home/youngcheese/revolve/experiments/jasper_experiments/data')


analysis = 'analysis_plane'
output_directory = paste(base_directory[2],'/',analysis ,sep='')

experiments_type = c('costplasticoding')
runs = list(c(1:1))
...
methods_labels = c( 'costplasticoding') # note that labels of Plane death and Tilted death are INVERTED on purpose, to fix the mistake done when naming the experiments.
  ...
  for(a in 1:length(aggregations)){
    ...
    for (out in outliers)
    {
      ...
      for(gc in gens_box_comparisons)
      {
        ...
        comps = list( c( 'Baseline','Plane death' ) ,
